...
export default class Dapp extends React.Component<Props, State> {
  ...
  async backerMint(courseTokenId: number, amount: number): Promise<void>
  {
    if (courseTokenId <=0 ) {
      this.setError("You must to select a course");
      return;
    }
    this.setError();
    try {
      await this.contract.backerMint(courseTokenId, {value: amount});
    } catch (e) {
      this.setError(e);
      var auxDapp = this;
      setTimeout(function (){
            ...
            {this.isContractReady() ?
              <>
                <CollectionStatus
                  ...
                  isMintEnabled={this.state.isMintEnabled}
                ...
                {this.state.totalSupply < this.state.maxSupply ?
                  <MintWidget
                    ...
                    backerMint={(courseTokenId: number, amount: number) => this.backerMint(courseTokenId, amount)}
  ...
  private async initWallet(): Promise<void>
  {
    ...
    this.setState({
      ...
      isMintEnabled: await this.contract.backerMintEnabled(),
